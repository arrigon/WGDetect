...
# Prepare blast, rapsearch and idx db for reference proteins (only if needed)
...
unless(-e "refs/$db.rapdb"){
  $command = "./bin/prerapsearch -d refs/$db -n refs/$db.rapdb";
  print "### $scriptname : $command\n";
  system("$command");
  }
...
foreach $genome (@list){
  ...
  my $dbload =-s "refs/$db.rapdb";
  $dbload = 1.5 * $dbload / 1e9;
